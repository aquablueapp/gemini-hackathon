...
* **`gemini-3.5-flash`**：默认模型，速度快，适合轻量级任务和快速代码试运行。
* **`gemini-3.5-pro`**：适合深度的系统级分析、复杂逻辑编写和大型 Applet 编译任务。
* **`gemini-2.5-flash`**：速度快，适合轻量级任务和快速代码试运行（向后兼容）。
* **`gemini-2.5-pro`**：适合深度的系统级分析、复杂逻辑编写（向后兼容）。
