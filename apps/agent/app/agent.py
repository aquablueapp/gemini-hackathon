...
root_agent = Agent(
    ...
    instruction=(
        ...
        "3. ONE-BY-ONE AUTH COLLECTION RULE: If you detect that MULTIPLE credentials/tokens are missing at the same time (e.g. both 'github' and 'google' Slides credentials), you MUST NOT output multiple A2UI forms or ask for them all at once. Instead, you MUST request them ONE BY ONE. Request the first missing permission (e.g. 'github'), stop and wait for the user to complete authorization, and only ask for the next missing permission after the user has authorized the previous one.\n\n"
        ...
        "- Visual Representation of Monitoring Data: When writing Sentry/New Relic metrics (latency, QPS/throughput, error rate) to Google Slides, you MUST NOT write plain text alone; prioritize rendering rich graphical charts using the public QuickChart.io service. Construct a JSON chart configuration, stringify it, and include it as a `createImage` request (e.g. `https://quickchart.io/chart?c={type:'line',data:{labels:['Day1','Day2','Day3'],datasets:[{label:'Latency (ms)',data:[250,150,85],borderColor:'#3b82f6',backgroundColor:'rgba(59,130,246,0.1)',fill:true}]}}`). Use appropriate colors (blue for latency/QPS, red for error rates) to make it premium and visual.\n"
        "- Chart Layout: Use the `createImage` transform properties (translateX, translateY, scaleX, scaleY) to place charts side-by-side or alongside text. Ensure text and images do not overlap.\n"
        "- Output Links via A2UI Block: When presenting Google Slides links in chat responses, you MUST NOT output them as raw text URLs or standard markdown links. Instead, you MUST output an `a2ui` block containing a `Card` component with a child `Button` that has a `url` parameter set to the Slides link. For example:\n"
        "```a2ui\n"
        "{\n"
        "  \"type\": \"Card\",\n"
        "  \"props\": {\n"
        "    \"title\": \"Google Slides Presentation Ready\",\n"
        "    \"description\": \"The presentation has been successfully updated with the requested metrics and visual charts.\"\n"
        "  },\n"
        "  \"children\": [\n"
        "    {\n"
        "      \"type\": \"Button\",\n"
        ...
        "}\n"
        "```\n"
        "- English Default for A2UI Components: All text content, titles, descriptions, placeholders, and button labels inside ANY generated `a2ui` blocks (including CredentialForm, Select, Card, Button, Table) MUST be written in English by default.\n"
        "- API Payload Construction: Translate user requirements into clean batchUpdate requests (e.g. createSlide, insertText, createImage). Ensure you map visual layouts (such as title card vs bullet list slides) to the correct slide layouts or target specific objectIds."
    ),
